...
create view laporan_anc as select `a`.`id_anc` AS `Id_Anc`,
c.tgl_periksa,
c.no_pasien,
`a`.`id_periksa` AS `Id_Periksa`,
`a`.`nm_suami` AS `Nama_Suami`,
concat(`a`.`tinggi_bdn`,' ',`sat_tinggi`.`Nama_Satuan`) AS `Tinggi_Badan`,
concat(`a`.`berat_bdn`,' ',`sat_berat`.`Nama_Satuan`) AS `Berat_Badan`,
`a`.`hpht` AS `hpht`,`a`.`htp` AS `htp`,
concat(`a`.`umur_khmln`,' ',`sat_umur`.`Nama_Satuan`) AS `Umur_Kehamilan`,
`a`.`kb_terakhir` AS `KB_Terakhir` 
from `tbl_anc` `a` join `daftar_satuan` `sat_tinggi` 
on `a`.`id_sat_tinggi` = `sat_tinggi`.`Id_Sat` join `daftar_satuan` `sat_berat` 
on `a`.`id_sat_berat` = `sat_berat`.`Id_Sat` join `daftar_satuan` `sat_umur` on `a`.`id_sat_umur` = `sat_umur`.`Id_Sat` 
join tbl_periksa c on a.id_periksa = c.id_periksa;
...
CREATE VIEW `laporan_tahunan` AS select `laporan_pemasukan`.`Tgl_Periksa` AS `Tgl_Periksa`,sum(`laporan_pemasukan`.`Total_Harga`) AS `Total_harga` from `laporan_pemasukan` where (year(`laporan_pemasukan`.`Tgl_Periksa`) = year(now())) group by month(`laporan_pemasukan`.`Tgl_Periksa`);
