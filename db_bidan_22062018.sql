...
CREATE TABLE `laporan_bulanan` (`id_periksa` varchar(50), `tgl_periksa` timestamp, `Total` decimal(52,0));


DROP VIEW IF EXISTS `laporan_harian`;
CREATE TABLE `laporan_harian` (`id_periksa` varchar(50), `tgl_periksa` timestamp, `Total` decimal(52,0));
...
CREATE TABLE `laporan_tahunan` (`bulan` int(2), `Total` decimal(52,0));
...
('1006181117590416',	'2018-06-10 09:19:24',	4,	'Peneruksaan pertama',	'120/80',	26),
('1006181122392383',	'2018-06-10 09:23:08',	5,	'Sakik pinggang e',	'120/80',	26),
('1106181056409573',	'2018-06-11 15:57:06',	4,	'asdsdsa',	'60/80',	26),
('110620181204439724',	'2018-06-10 10:05:05',	4,	'Pemeriksaan/Pemasangan KB',	'120/120',	26),
('110620181206354178',	'2018-06-10 10:07:07',	4,	'Pemeriksaan/Pemasangan KB',	'120/80',	26),
('110620181214334910',	'2018-06-10 10:14:58',	4,	'Pemeriksaan/Pemasangan KB',	'60/80',	26)
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `laporan_harian` AS select `a`.`Id_Periksa` AS `id_periksa`,`b`.`tgl_periksa` AS `tgl_periksa`,sum((`a`.`Jumlah` * `a`.`Total`)) AS `Total` from (`laporan_terapi` `a` join `tbl_periksa` `b` on((`a`.`Id_Periksa` = `b`.`id_periksa`))) group by `a`.`Id_Periksa`;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `laporan_tahunan` AS select month(`laporan_harian`.`tgl_periksa`) AS `bulan`,`laporan_harian`.`Total` AS `Total` from `laporan_harian` where (year(`laporan_harian`.`tgl_periksa`) = year(now())) group by year(`laporan_harian`.`tgl_periksa`);
